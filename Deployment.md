...
- **Cloud Build - [Helm tool builder](https://github.com/GoogleCloudPlatform/cloud-builders-community/tree/master/helm)**
  ...
  - Each Cloud Build pipeline script can contain various **helm** commands as the build step
    ...
    > **Note**:
    >
    > This is optional. Any other build tool can be used instead of Cloud Build; buit this deployment uses Cloud Build heavily.
    >
    > Helm tool installer is a tool that works with Cloud Build and helps to automte running helm charts as part of the deployment process.
...
- **Open-Network-Aggregator (*Root Folder*)**
  ...
  - **backend**
    ...
    - **utilities** - Contains helper services used across the solution
      - **storage** - A Wrapper around GCS APIs; supports various cloud storage operatons viz.  *download a file*, *upload a file* etc.
      - **event-sockets** - Contains Socket.IO Server and Client for accepting asynchronous callbacks from various Open Networks or Content Providers
      - **websock-streamer** -  Contains Socket.IO Server and Client for accepting streaming content generated by various generative AI services. In this solution - *Text generation* and **Multi-modal Content generaion** - are two services which use this service
...
GSA_DISPLAY_NAME= <Display name of the Service Account for the Project>
...
DOMAIN1_NAME=<dev environment to be exposed throigh GCP LB>
DOMAIN2_NAME=<staging environment to be exposed throigh GCP LB>
DOMAIN3_NAME=<prod environment to be exposed throigh GCP LB>
DOMAIN4_NAME=<Event websocket to be exposed throigh GCP LB>
DOMAIN5_NAME=<Stream websocket to be exposed throigh GCP LB>
