...
{    
    EnamMandiAdapter: "enam-mandi-adapter"
}

const KEnamMandiDomain = "integrator:enam-mandi";
...
{
    _allUrls[KMicroServices.EnamMandiAdapter] = `${process.env.ENAM_MANDI_ADAPTER_URL}`;
}
...
function prepareEnamMandiMessage(nlpInfo)
{
    const enamMandiMessage = {};
    enamMandiMessage.domain = KEnamMandiDomain;
    enamMandiMessage.transaction_id = nlpInfo.transactionId;
    enamMandiMessage.message_id = nlpInfo.messageId;
    enamMandiMessage.network = nlpInfo.network;    
    return enamMandiMessage;
}
...
_express.post("/search", async (request, response) =>
{
    ...
    const enamMandiMessage = prepareEnamMandiMessage(nlpInfo);

    const requestOptions = {};
    requestOptions.httpsAgent = _axiosAgent;

    const requestBody = enamMandiMessage;
    const results = {};
    
    try
    {
        const adapterResponse = await Axios.post(`${_allUrls[KMicroServices.EnamMandiAdapter]}/enam`,
